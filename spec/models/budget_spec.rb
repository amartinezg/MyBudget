...
RSpec.describe Budget, type: :model do
  ...
  describe "ActiveModel validations" do
    ...
    it { is_expected.not_to validate_inclusion_of(:type).in_array(%w[Income Expense])}
  ...
  describe "AASM validations" do
    describe "with valid budget" do
      ...
      it "should run when period belongs to current month" do
        allow(Date).to receive_messages(current: Date.current.beginning_of_month.next_month.to_date)
        expect(budget).to allow_event(:run)
        expect(budget).not_to allow_event(:close_budget)
    ...
    describe "with expired budget" do
      ...
      it "should close when period belongs to last month" do
        ...
        expect(expired_budget).to allow_event(:close_budget)
        expect(expired_budget).to transition_from(:running).to(:closed).on_event(:close_budget)
      ...
      it "should not close when period does not belongs to last month" do
        allow(Date).to receive_messages(current: Date.current.beginning_of_month.last_month.to_date)
